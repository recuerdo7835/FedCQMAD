...
def dirichlet_split_indices(
    ...
    for k in range(n_classes):
        ...
        if len(indices) == 0:
            continue
        rng.shuffle(indices)
        # 从 Dir(alpha, alpha, ..., alpha) 采样比例，再用 multinomial 确定各客户端样本数
...
if __name__ == "__main__":
    ...
    # 测试 Non-IID（Dirichlet α=0.5，5 客户端）
    print("\n=== Non-IID 模式（Dirichlet α=0.5, 5 客户端）===")
    n_classes = int(np.max(Y_train)) + 1
    for cid in range(5):
        X_c, y_c, _, _ = get_data(client_id=cid, num_clients=10, alpha=0.5)
        counts = np.bincount(y_c, minlength=n_classes)
        print(f"  Client {cid}: {len(y_c)} 样本, 各类别分布 {counts.tolist()}")
