"""
FedCQMAD 本地训练 - 灵活投影维度与量子比特数（方法3: 线性映射扩展）
支持任意 proj_dim 与 n_qubits 组合，例如：
  proj_dim=8 -> n_qubits=4/6/8/10
  proj_dim=4/8/12 -> n_qubits=8
用法：PROJ_DIM=8 N_QUBITS=6 python QCNN_flexible.py
"""
import os
import torch
import numpy as np
...
CONV_OUTPUT_SIZE = 32 * 79 * 2  # MalDroid
DEFAULT_PROJ_DIM = 4
DEFAULT_N_QUBITS = 8
...
if __name__ == "__main__":
    ...
    print("QCNN (灵活投影+量子比特) - MalDroid-2020")
    print(f"proj_dim={proj_dim}, n_qubits={n_qubits}")
    print(f"PyTorch: {torch.__version__}, CUDA: {torch.cuda.is_available()}")

    trainer = Trainer(num_classes=5, proj_dim=proj_dim, n_qubits=n_qubits)
