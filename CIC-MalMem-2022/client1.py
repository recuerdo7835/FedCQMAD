...
class FedProxClient(fl.client.NumPyClient):
    ...
    def evaluate(self, parameters, config):
        ...
        with torch.no_grad():
            for i in tqdm(
                range(0, len(self.X_test), batch_size),
                total=num_batches,
                desc="Evaluating",
            ):
                inputs = torch.tensor(
    ...
    def _plot_confusion_matrix(self, cm, accuracy, precision, recall, f1):
        ...
        plt.title(
            f"FedProx MalMem Confusion Matrix\nAccuracy: {accuracy:.4f}, "
        ...
        save_path = os.path.join(results_dir, "final_confusion_matrix_fedprox.png")
...
if __name__ == "__main__":
    ...
    client_instance = FedProxClient(client_id=client_id, num_clients=num_clients, alpha=alpha, iid=iid)
